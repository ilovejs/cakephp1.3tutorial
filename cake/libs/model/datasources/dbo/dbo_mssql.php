			...
			for ($i = 0; $i < $count; $i++) {
				...
				if (!preg_match('/\s+AS\s+/i', $fields[$i])) {
					...
					if (strpos($fields[$i], '.') === false) {
						...
						if ($fieldDescription[$fields[$i]]['type'] == 'nvarchar') {
							if ($fieldDescription[$fields[$i]]['length'] != -1) {
								$fieldName = "CAST(CAST(".$this->name($alias . '.' . $fields[$i])." AS VARCHAR(". $fieldDescription[$fields[$i]]['length'] . ")) AS TEXT)";
							
							} else {
								$fieldName = "CAST(CAST(".$this->name($alias . '.' . $fields[$i])." AS VARCHAR(MAX)) AS TEXT)";
							}
						...
						if (array_key_exists($build[1],$fieldDescription) && $fieldDescription[$build[1]]['type'] == 'nvarchar') {
							if ($fieldDescription[$build[1]]['length'] != -1) {
								$fieldName = "CAST(CAST(".$this->name($build[0] . '.' . $build[1])." AS VARCHAR(". $fieldDescription[$build[1]]['length'] . ")) AS TEXT)";
							
							} else {
								$fieldName = "CAST(CAST(".$this->name($build[0] . '.' . $build[1])." AS VARCHAR(MAX)) AS TEXT)";
							}
							
						} else {
							$fieldName  = $this->name($build[0] . '.' . $build[1]);
						}
						//echo 'fieldName is: ' . $fieldName . '<br>';
 ...
 */
	function column($real) {
		...
		/* 20110831: Hacking core to support nVarchar, nText type */
		if ($col == 'nvarchar') {
			return 'nvarchar';
		}
		if ($col == 'ntext') {
